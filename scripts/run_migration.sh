...
# 步骤 1: 检查并准备数据（如果还没有）
echo "[步骤 1/7] 检查数据文件..."
if [ ! -f "$DATA_DIR/parsing/tests_tf.parsed.jsonl" ]; then
    echo "  执行数据准备..."
    python3 main.py
    python3 core/mapping/api_mapping.py
    echo "  ✓ 数据准备完成"
else
    echo "  ✓ 数据文件已存在，跳过准备步骤"
fi
...
if [ ! -f "$DATA_DIR/migration/migration_candidates_fuzzy.jsonl" ]; then
    echo "  执行识别..."
    python3 component/migration/migrate_identify_fuzzy.py
    echo "  ✓ 识别完成"
else
    CANDIDATE_COUNT=$(wc -l < "$DATA_DIR/migration/migration_candidates_fuzzy.jsonl" | tr -d ' ')
...
if Path('$DATA_DIR/migration/migration_candidates_fuzzy.jsonl').exists():
...
echo "  - 执行结果: $DATA_DIR/results/migrate_exec.jsonl"
echo "  - 报告文件: reports/"
echo ""
