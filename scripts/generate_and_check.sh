...
results = []
if Path('$DATA_DIR/migrate_exec.jsonl').exists():
    results = [json.loads(l) for l in open('$DATA_DIR/migrate_exec.jsonl')]
elif Path('$DATA_DIR/migrate_comparison.jsonl').exists():
    results = [json.loads(l) for l in open('$DATA_DIR/migrate_comparison.jsonl')]
...
echo "----------------------------------------"
if [ -f "$DATA_DIR/migrate_comparison.jsonl" ]; then
    python3 component/migrate_report.py \
        --comparison $DATA_DIR/migrate_comparison.jsonl \
        --output $REPORTS_DIR/migration_comparison.html
    echo "  [OK] 报告已生成: $REPORTS_DIR/migration_comparison.html"
elif [ -f "$DATA_DIR/migrate_exec.jsonl" ]; then
    echo "  [INFO] 只有执行结果，无法生成对比报告"
    echo "  如需对比报告，请运行:"
    echo "    python3 component/migrate_compare.py --migrated-dir $MIGRATED_DIR --limit $LIMIT"
else
    echo "  [WARN] 未找到结果文件"
fi
echo ""
...
echo "  2. 执行结果: $DATA_DIR/migrate_exec.jsonl"
